...
import { parseAsrText, selectEngine, transcribeWithLlama } from "./engine";
...
describe("transcribeWithLlama", () => {
  ...
  it("posts the wav as multipart with the model and returns English untouched", async () => {
    const { fetchImpl, calls } = fakeFetch({});
    const result = await transcribeWithLlama(base(fetchImpl));
    expect(result).toEqual({ ok: true, text: "hello", language: "English", translated: false });
  ...
  it("translates non-English through the chat endpoint with thinking off", async () => {
    const { fetchImpl, calls } = fakeFetch({
      ...
      chat: (body) => {
        ...
        expect(b.model).toBe("gemma-4-e2b");
    ...
    const result = await transcribeWithLlama(base(fetchImpl));
    expect(result).toEqual({ ok: true, text: "Dude, yesterday", language: "Hindi", translated: true });
    expect(calls.map((c) => c.url.split("/v1/")[1])).toEqual(["audio/transcriptions", "chat/completions"]);
  });

  it("skips translation when disabled, when the text is empty, or when the budget is nearly spent", async () => {
    const hindi = () => new Response(JSON.stringify({ text: "language Hindi<asr_text>नमस्ते" }));
    let f = fakeFetch({ asr: hindi });
    expect(await transcribeWithLlama(base(f.fetchImpl, { translate: false }))).toMatchObject({ text: "नमस्ते", translated: false });
    expect(f.calls).toHaveLength(1);

    f = fakeFetch({ asr: hindi });
    expect(await transcribeWithLlama(base(f.fetchImpl, { remainingMs: () => 900 }))).toMatchObject({ text: "नमस्ते", translated: false });
    expect(f.calls).toHaveLength(1);

    f = fakeFetch({ asr: () => new Response(JSON.stringify({ text: "language Hindi<asr_text>" })) });
    expect(await transcribeWithLlama(base(f.fetchImpl))).toMatchObject({ text: "", translated: false });
    expect(f.calls).toHaveLength(1);
  });

  it("falls back to the raw transcript when translation fails or returns nothing", async () => {
    const hindi = () => new Response(JSON.stringify({ text: "language Hindi<asr_text>नमस्ते" }));
    let f = fakeFetch({ asr: hindi, chat: () => new Response("boom", { status: 500 }) });
    expect(await transcribeWithLlama(base(f.fetchImpl))).toMatchObject({ ok: true, text: "नमस्ते", translated: false });
