...
Measured in the real bb app on a 20-core CPU with no GPU (Gemma 4 26B-A4B polisher): the text lands 2.3–2.9 s after you stop for a 9 s Hinglish take and 4.1–4.7 s for a 60 s one; with the E4B polisher 2.2–2.7 s and 3.5 s. The old whole-clip path took ~5 s for the short take and could not finish the long one inside bb's 10 s cap.
...
The browser hands the plugin a slice of audio every second. The host decodes what has arrived so far (ffmpeg copes with a truncated container), and every 4–6 s of speech is cut **at a pause** and sent to Qwen3-ASR on one of four parallel slots — all while you are still talking. Each chunk gets the previous chunk's text as its prompt, which keeps script and vocabulary steady across cuts in code-switched speech (without it, a Hinglish tail came back as "BJ, do please"; with it, "भेज दो प्लीज"). Runs of chunks that end at a long pause (≥ 450 ms, a finished thought) — or that have grown past 9 s — are polished as soon as they are recognised. When you stop, a long tail is split at pauses so its pieces run in parallel, and only that plus the last group's polish remain. Chunking also fixes a weakness of the whole-clip approach: on a 34 s mixed Hindi/English clip Qwen3-ASR given the whole file duplicated a sentence and dropped the English one entirely; the chunked pass got the structure right. Every slice is written to SQLite as it arrives; a failed transcription keeps the audio and the row, and Retry sends the same audio down the same path.
...
3. **Recognise** — each chunk is a wav to `POST /v1/audio/transcriptions` on Qwen3-ASR-1.7B with the previous chunk's text as `prompt`, up to four in flight (`np = 4` in `models.ini`). It answers in the spoken language with a prefix: `language Hindi<asr_text>यार कल का डिप्लॉय…`.
